...
export let graph = (function () {
    ...
    return {
        rootElId: null,
        setRootElId: function(id) {
            this.rootElId = id;
        },
        getGraphEl: function() {
            let svg = d3.select(`#${this.rootElId}`);
            return {
                svg,
                container: svg.select('g'),
                links: svg.select('.links'),
                nodes: svg.select('.nodes'),
                labelNodes: svg.select('.labelNodes')
            }
        },
        ...
        init: function() {
            ...
            // Initialize graph svg attributes
            const svg = d3.select(`#${this.rootElId}`)
                            .attr('width', width)
                            .attr('height', height);
            
            // Initialize graph groups elements and classify
